...
// Dual masked range-histogram. Returns {histA, histB} (int32, length binNum),
// plus validCount ([1] int32) when emitValidCount is true. base may be nullptr.
MNN_PUBLIC std::vector<VARP> _DualRangeHist(VARP a, VARP b, VARP base, int binNum, float low, float high,
                                            int sampleStride = 1, bool emitValidCount = true);
